...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        if has_gt:
            ...
            cnf_matrix = confusion_matrix(np.ndarray.flatten(pred), np.ndarray.flatten(label))
            np.set_printoptions(precision=5)
            class_names = ['tumor','non-tumor','background']

            # Plot Confusion Matrix
            plot_confusion_matrix(cnf_matrix, classes=class_names, normalize=True,title = "DRN")
            ###############
